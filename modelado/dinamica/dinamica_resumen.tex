...
Velocidad de plegado tras el golpe & 8.4 rad/s = 479\(^\circ\)/s & \eqref{eq:dthimp} \\
Energía por servo, antes \(\to\) después del golpe & 0.911 \(\to\) 0.663 J & \eqref{eq:Kimp} \\
Energía que suma la gravedad al plegar, \(\Delta V\) & 0.487 J & \eqref{eq:Eabs} \\
Energía a disipar por servo, \(E_{abs}\) & 1.149 J & \eqref{eq:Eabs} \\
\textbf{Par medio necesario} \(\bar\tau\) & \textbf{2.20 N\,m = 22.4 kg\,cm} & \eqref{eq:taumedio} \\
Par disponible (DS3225MG) & 21 kg\,cm & \\
Escalón máximo absorbible \(h_{max}\) & 16 cm & \eqref{eq:hmax} \\
Fuerza contra el piso mientras el servo frena & 10 N por rueda, 2.0 veces la estática & \eqref{eq:N} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Con 18 cm no alcanza.} Hacen falta 22.4 kg\,cm contra los 21 del servo, así que la pata llega al tope
de plegado todavía a 2.0 rad/s, con el 11 por ciento de la energía del golpe sin disipar. Esos 0.074 J por
rueda se los come el neumático: con la rigidez supuesta de 30 kN/m se hunde 2.2 mm y recibe 67 N, trece veces
la carga estática. El límite de este servo queda en 16 cm.

Dos números más que conviene mirar. La pata se pliega a \(479^\circ\)/s, más rápido que la velocidad en vacío
del propio servo, que ronda los \(400^\circ\)/s: en el aterrizaje el servo no manda, lo manda el golpe, y solo
puede frenar. Y mientras el servo frena, la fuerza contra el piso se queda en dos veces la carga estática, o
sea que la pata cumple bien su función de suspensión; el pico grande aparece recién al tocar el tope.

Para bajar los 18 cm hay cuatro caminos, de menor a mayor cambio: limitar el escalón a 15 cm; agregar el
elemento elástico en serie del documento de dimensionamiento, que alarga la carrera efectiva sin pedirle más
par al servo; usar un neumático más blando, que baja el pico del tope pero no el par medio; o pasar a un servo
de más par.
...
\item \texttt{dinamica\_pata.m}: script de MATLAB con toda la dinámica. Sección 1: los parámetros de la tabla,
  ...
  escalón, con la altura en la variable 	exttt{h\_escalon}. Las funciones

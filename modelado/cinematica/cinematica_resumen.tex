...
\begin{center}
\includegraphics[width=0.98\textwidth]{../geometria/geometria_robot.png}
\end{center}
...
\midrule
\(AB\) & bancada: distancia entre los pivotes \(A\) y \(B\) de la cabina & 80 mm & \texttt{cabeza\_v31} \\
\(\beta\) & ángulo de \(A\!\to\!B\) desde \(+x\) & \(45^\circ\) & \texttt{cabeza\_v31} \\
\(AD\) & manivela (la mueve el servo) & 112 mm & \texttt{eslabon\_AD\_v2} \\
\(BC\) & balancín & 108 mm & \texttt{eslabon\_BC} \\
\(CD\) & acoplador, tramo \(C\)--\(D\) & 40.8 mm & \texttt{eslabon\_CDP\_v2} \\
\(DP\) & acoplador, tramo \(D\)--\(P\) (hasta el eje de rueda) & 112 mm & \texttt{eslabon\_CDP\_v2} \\
...
Cotas de la segunda iteración del CAD (\texttt{diseño\_mecanico/segunda\_iteracion}, STEP del 14/9/2026), medidas sobre
los STEP de cada pieza y del ensamble. La bancada se toma como 80 mm a \(45^\circ\), que es la intención del diseño; en
el STEP \(B\) está a 77.9 mm y \(46.6^\circ\) de \(A\) (a la altura justa, 3 mm corto en horizontal), pendiente de corregir.
...
\midrule
320 & 131.8 & 102.8 & 46.7 & 56.1 & (85.8, \(-72.0\)) & (108.5, \(-38.1\)) & (0.2, \(-144.2\)) & 177.2 \\
335 & 113.2 & 113.4 & 72.3 & 41.1 & (101.5, \(-47.3\)) & (132.2, \(-20.5\)) & (0.1, \(-94.9\)) & 127.9 \\
350 & 93.1 & 125.3 & 100.1 & 25.1 & (110.3, \(-19.4\)) & (147.2, \(-2.1\)) & (\(-0.3\), \(-37.2\)) & 70.2 \\
\bottomrule
\end{tabular}
\end{center}
...
\item En todo el recorrido \(BD\in[93,\,132]\) mm, dentro de \([\,|BC-CD|,\,BC+CD\,]=[67,\,149]\) mm:
...
\item Carrera del eje de rueda entre \(320^\circ\) y \(350^\circ\): \(107.0\) mm vertical, con un desvío
horizontal de sólo \(0.5\) mm. Con la bancada a escala con las barras (\(AB=80\) mm a \(45^\circ\)) el mecanismo
hace la línea recta para la que fue diseñado (en la primera iteración, con barras de escala 100 y bancada de
80 mm a \(47.5^\circ\), el desvío era de 9 mm).
\end{itemize}
...
\subsection*{Archivos}
\begin{itemize}[nosep]
\item \texttt{modelado/geometria/geometria\_robot.png} (y \texttt{.py}): figura de la geometría con nombres y valores.
\item \texttt{modelado/cinematica/parametros\_geometria.m}: las cotas, leídas de \texttt{modelado/parametros/parametros\_fisicos.m} (la única fuente).

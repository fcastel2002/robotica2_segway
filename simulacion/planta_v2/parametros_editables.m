...
%PARAMETROS_EDITABLES  Los valores que el ingeniero cambia. ESTE es el archivo que se edita.
%
%   Unidades de entrada: mm, g, grados (comodas para CAD y balanza); donde se indica, SI.
%   Cada linea dice de donde salio el valor y que afecta. parametros_robot.m lee este archivo,
%   convierte a SI y calcula lo derivado (m_b, J_b, CoM, tabla theta<->l, etc.).
%
...
%              'corregido' = bancada 100 mm a 45 grados y motor 280 rpm (lo que la simulacion pide).
  ...
  switch lower(variante)
    ...
    otherwise
      error('parametros_editables: variante "%s" no definida (usar ''cad'' o ''corregido'')', variante);
  end

function E = escenarios_robot(nombre, P)
%ESCENARIOS_ROBOT  Estado inicial, referencias, perturbaciones y overrides de cada escenario.
%   E = escenarios_robot('escalera', P)      lista = escenarios_robot('lista')
...
%   Los escenarios de escalera bajan 'piso_n' escalones de 16 x 30 cm a partir de x = 0.5 m; el robot
%   avanza a velocidad constante (0.15 m/s en 'cad', que no puede mas; 0.4 m/s en 'corregido').
  lista = {'equilibrio_3','equilibrio_8','equilibrio_15','empujon_3N','empujon_6N','agachar','avanzar', ...
           ...
           'sensor_retardo_10','sin_encoder','escalon_1','escalera','escalera_flexor'};
  if strcmp(nombre, 'lista'), E = lista; return; end
  d2r = pi/180; dt = 1e-3;
  if strcmp(P.variante, 'cad'), v_ap = 0.15; else, v_ap = 0.40; end
  E.nombre = nombre; E.tf = 6; E.overrides = {}; E.n_delay = P.sens.n_delay; E.semilla = P.sens.semilla; E.v_ap = v_ap;
  X0 = estado_inicial(P);
  switch nombre
    case {'escalera','escalera_flexor'}, E.tf = 1 + (P.piso.x0 + 3*P.piso.ancho + 0.4)/v_ap + 1;
    case 'escalon_1',                    E.tf = 1 + (P.piso.x0 + P.piso.ancho + 0.4)/v_ap + 1;
  end
  E.tf = ceil(E.tf);
  t = (0:dt:E.tf)';
  ...
  switch nombre
    case 'equilibrio_3',  X0(3) = 3*d2r;
    case 'equilibrio_8',  X0(3) = 8*d2r;
    case 'equilibrio_15', X0(3) = 15*d2r;
    ...
    case 'escalon_1',     x_ref = v_ap*max(t - 1, 0); E.overrides = {'piso_n', 1};
    case 'escalera',      x_ref = v_ap*max(t - 1, 0); E.overrides = {'piso_n', 3};
    case 'escalera_flexor', x_ref = v_ap*max(t - 1, 0); E.overrides = {'piso_n', 3, 'flexor', true};
    otherwise, error('escenarios_robot: escenario "%s" no definido', nombre);
  end
  E.X0 = X0;
